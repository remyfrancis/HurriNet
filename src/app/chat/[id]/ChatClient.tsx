...
export default function ChatClient({ chatId }: ChatClientProps) {
  ...
  const fetchChatData = async () => {
    ...
    try {
      ...
      // Fetch chat session
      const sessionResponse = await fetch(`/api/chats/sessions/${chatId}`, {
        headers: {
      ...
      // Fetch messages
      const messagesResponse = await fetch(`/api/chats/messages?session=${chatId}`, {
        headers: {
          Authorization: authToken,
        },
      });
  ...
  const initWebSocket = () => {
    ...
    const wsUrl = `${wsProtocol}//${window.location.hostname}:8000/ws/chat/${chatId}/?token=${authToken}`;
