...
interface EmergencyTeam {
  ...
  status: 'DEPLOYED' | 'STANDBY' | 'OFF_DUTY'
  equipment: string[]
}

export function EmergencyTeams() {
  const { token } = useAuth()
  ...
  const fetchTeams = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/teams/teams/emergency/`, {
        headers: {
      ...
      toast({
        title: 'Error',
        description: 'Failed to fetch emergency teams',
        variant: 'destructive'
      })
    }
  }
  ...
  const fetchAvailablePersonnel = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/teams/teams/emergency_personnel/`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleCreateTeam = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Emergency response team created successfully'
        })
        setIsCreateDialogOpen(false)
        setFormData({ name: '', type: 'FIRST_RESPONSE', leader_id: '' })
        fetchTeams()
      }
    } catch (error) {
      console.error('Error creating emergency team:', error)
      toast({
        title: 'Error',
        description: 'Failed to create emergency team',
        variant: 'destructive'
      })
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold">Emergency Response Teams</h2>
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Create Emergency Response Team</DialogTitle>
            </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="SEARCH_RESCUE">Search and Rescue</SelectItem>
        ...
        {teams.map((team) => (
          <Card key={team.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <p className="text-sm font-medium text-gray-500">Team Type</p>
                  <p>{(team.type || '').replace(/_/g, ' ')}</p>
                </div>
                <div>
