...
const SUPPLY_CATEGORIES = ["Bedding", "Sanitation", "Clothing", "Administrative", "Nutrition"]
...
export default function NonMedicalSupplies() {
  ...
  const [formData, setFormData] = useState<Partial<Supply>>({
    ...
    category: '',
    status: 'in_stock',
    is_medical: false // Set default to false for non-medical supplies
  })
  ...
  const fetchSupplies = async () => {
    try {
      ...
      // Filter for non-medical supplies only
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ ...formData, is_medical: false })
      })
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Non-Medical Supplies</h1>
